...
### Fixed
- **Font Loading**: Completely rewrote font loading mechanism to load font data into memory
  - Eliminated "unknown file format" errors when package is installed
  - Now loads font as binary data rather than file path for better reliability
  - Works consistently across all installation methods and platforms
- **Resource Access**: Improved resource loading fallback chain for better compatibility

### Technical Details
- Font data is now loaded into memory during initialization
- Uses `io.BytesIO` to create font from binary data
- Maintains all fallback strategies for different Python versions
